...
import { LoginRequest, ValidationProblemDetails } from '../lib/api/data-contracts';
...
const nonAdminLogin = computed(() => submitted.value && token.value && user.value && user.value.role !== 'Administrator')
	...
	await useApi(
		...
		error => {
			...
			switch (error.status) {
				...
				case 422:
					const validationProblemDetails = error.error as ValidationProblemDetails
...
<template>
	<div class="loginContainer">
		...
		<p class="errorText">{{ nonAdminLogin ? 'Must be Admin to log in' : errorText }}</p>
	</div>
</template>
